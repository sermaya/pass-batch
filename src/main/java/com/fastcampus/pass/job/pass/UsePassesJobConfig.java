...
public class UsePassesJobConfig {
    ...
    public JpaCursorItemReader<BookingEntity> usePassesItemReader() {
        String query = "select b from BookingEntity b join fetch b.passEntity " +
    ...
    // 이 프로젝트에서는 적합하지 않지만, ItemProcessor의 수행이 오래 걸려 병목 현상이 생기는 경우에 AsyncItemProcessor,
    // AsyncItemWritee를 사용하면 성능을 향상시킬 수 있습니다.
    @Bean
    ...
    public ItemProcessor<BookingEntity, BookingEntity> usePassesItemProcessor() {
        return bookingEntity -> {
            ...
            passEntity.setRemainingCount(passEntity.getRemainingCount());
    ...
    public AsyncItemWriter<BookingEntity> usePassesAsyncItemWriter() {
        ...
        // useOassesItemWriter() 최종 결과값을 넘겨주고 작업을 위임합니다.
    ...
    public ItemWriter<BookingEntity> usePassesItemWriter() {
        return bookingEntities -> {
            for (BookingEntity bookingEntity : bookingEntities) {
                // 잔여 횟수를 업데이트 합니다.
                int updatedCount =
                        passRepository.updateRemainingCount(
                                bookingEntity.getPassSeq(),
                                bookingEntity.getPassEntity().getRemainingCount());

                // 진여 횟수가 업데이트 완료되면, 이용권 사용 여부를 업데이트합니다.
                if (updatedCount > 0) {
